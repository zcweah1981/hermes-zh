...
function formatPageLinks(pages: Awaited<ReturnType<typeof loadPagesManifest>>) {
  return pages
    .filter((page) => page.status === 'published')
    .map((page) => `- ${page.title}：${buildCanonicalUrl(toDocPath(page.slug))}（${sanitizeLlmsDescription(getDocsSeoDescription(page, toDocPath(page.slug)))}）`)
    .join('\n')
}

function formatPackLinks(packs: Awaited<ReturnType<typeof loadPacksManifest>>) {
  return packs
    .filter((pack) => pack.status === 'published')
    .map((pack) => `- ${pack.title} Pack：${buildCanonicalUrl(`/packs/${pack.id}`)}${pack.summary ? `（${pack.summary}）` : ''}`)
    .join('\n')
}

export async function GET() {
  const [pages, packs] = await Promise.all([loadPagesManifest(), loadPacksManifest()])
  const pageLinks = formatPageLinks(pages)
  const packLinks = formatPackLinks(packs)

  const llmsText = `# Hermes Agent 中文站
...
- 实战应用：https://hermes-zh.com/docs/start/practical

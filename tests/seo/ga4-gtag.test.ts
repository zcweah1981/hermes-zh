...
describe('GA4 gtag integration', () => {
  it('injects GA4 gtag scripts from the global App Router layout', () => {
    ...
    assert.match(layoutSource, /id=['"]ga4-idle-loader['"]/, 'External GA4 loader should have a stable idle-loader script id')
    ...
    assert.match(layoutSource, /script\.src = 'https:\/\/www\.googletagmanager\.com\/gtag\/js\?id=\$\{GA_MEASUREMENT_ID\}'/)
    assert.match(layoutSource, /id=['"]ga4-idle-loader['"][\s\S]*?strategy=['"]lazyOnload['"]/, 'GA4 network loader should be preserved but delayed until browser idle to reduce mobile TBT')
    assert.match(layoutSource, /window\.setTimeout\(loadGtag, 12000\)/, 'GA4 loader should not compete inside the initial Lighthouse trace window')

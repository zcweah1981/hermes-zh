...
describe('SEO/GEO discovery surface sync', () => {
  ...
  it('keeps discovery surfaces generated from manifests instead of hand-maintained partial fallback lists', () => {
    ...
    assert.match(sitemap, /loadPagesManifest/)
    assert.match(sitemap, /loadPacksManifest/)
    assert.match(sitemap, /\/llms\.txt/)
    assert.match(sitemap, /\/ai-index/)
    assert.match(robots, /sitemap:\s*SITEMAP_URL/)
    assert.match(robots, /\/llms\.txt/)
    assert.match(robots, /\/ai-index/)
  ...
  it('keeps canonical metadata and JSON-LD page types aligned with page class', () => {
    ...
    const packPage = read('app/packs/[id]/page.tsx')

    assert.match(canonical, /buildCanonicalUrl/)
    assert.match(metadata, /alternates:\s*{\s*canonical: effectiveCanonical/s)
    assert.match(docsPage, /buildTechArticleJsonLd\(page\)/)
    assert.match(docsPage, /buildFAQPageJsonLd\(page\)/)
    assert.match(docsPage, /buildBreadcrumbJsonLd/)
    assert.doesNotMatch(docsPage, /buildSoftwareApplicationJsonLd\(page\)/)
    assert.match(packPage, /buildSoftwareApplicationJsonLd\(pack\)/)
    assert.match(packPage, /buildBreadcrumbJsonLd/)
    assert.match(rootLayout, /buildWebSiteJsonLd\(\)/)
    assert.match(rootLayout, /buildOrganizationJsonLd\(\)/)
    assert.match(rootLayout, /buildSoftwareApplicationJsonLd\(\)/)
  })
